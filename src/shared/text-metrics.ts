...
/**
 * Default conversational filler phrases. The canonical list now lives
 * in `filler-categories.ts` as a categorised structure used by the
 * Settings UI; this aliased re-export keeps the flat-array contract
 * for downstream callers (`buildFillers`, `cache.ts`, demo data, etc.).
 *
 * Spans the usual range — interjections, conversational fillers,
 * softeners, emphasis words, uncertainty + corrections, and vague
 * references / trailing closers. Users prune what doesn't suit them
 * from Settings → Analysis → Filler dictionary.
 */
import { DEFAULT_FILLER_PHRASES_FROM_CATEGORIES } from './filler-categories'

export { DEFAULT_FILLER_CATEGORIES, type FillerCategory } from './filler-categories'
export const DEFAULT_FILLER_PHRASES: readonly string[] = DEFAULT_FILLER_PHRASES_FROM_CATEGORIES
